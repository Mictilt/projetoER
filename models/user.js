...
exports.userFindById = (id, cb) => {
    User.findById(id, {  _id:1, username:1, password:1, email:1, tipo:1})
        .exec()
        .then(doc => cb(doc))
...
exports.list = (cb) => {
    ...
    User.find({ }, { _id:1, username:1, password:1, email:1, tipo:1})
        .exec()
        .then((docs) => cb(docs))
        .catch(err => cb(err));
};
...
exports.patchUser = (id, userData, cb) => {
    ...
    User.findOneAndUpdate({_id: id}, userData, {new:true, overwrite:true, projection: {  _id:0, username:0, password:0, email:0, tipo:1}})
        .exec()
        .then(() => cb())
        .catch(err => cb(err));
};
