...
exports.TicketgetById = (req, res) => {
    ticketModel.TicketFindById(req.params.id, (doc, err) => {
        const isAuthenticated = !!req.user;
        const user = req.user;
        if(!err) res.status(200).render('ticketsEspecifico',{isAuthenticated,tickets:doc,user});
        else res.status(500).send({message: err.message});
    });
};
